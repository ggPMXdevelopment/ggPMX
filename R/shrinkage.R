...
shrinkage <-
  function(par_est,ind_pred,sys="mlx",fun="sd"){
      dx1 <- par_est[grepl("omega",PARAM),c("VAR","EFFECT"):=tstrsplit(PARAM,"_")]
      dx1 <- dx1[,c("EFFECT","VAR"):=list(str_trim(tolower(EFFECT)),str_trim(VAR))]
      dx1 <- dx1[ VAR=="omega"]
      dx2 <- ind_pred[VAR=="eta" & grepl("mean",FUN)]
      unique(merge(dx2 ,dx1,by="EFFECT")[,list(SHRINK=1-get(fun)(VALUE.x)/VALUE.y),EFFECT])
  }
