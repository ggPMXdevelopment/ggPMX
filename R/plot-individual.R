...
#'
#'
individual <- function(labels,has.curve=TRUE,facets=list(ncol=3,nrow=4),...){
  if(missing(labels))
    labels <- list(
...
plot_pmx.individual <-
  function(x,dx,include){
    ...
    with(x,
         {
           p <- ggplot(dat, aes(TIME, DV,linetype=variable))+
             with(point,geom_point(shape=shape, color=color,size=size))
           if(has.curves)
             p <- p + geom_line(aes(y=value),size=1)

           p <- plot_pmx(gp,p)

           ## split pages
           npages <- ceiling(with(facets,length(unique(dat$ID)/nrow/ncol)))
           with(facets,
                lapply(seq_len(npages),function(i)
                  p <- p + facet_wrap_paginate(~ID,ncol=ncol,nrow=nrow,page=i)
                ))
         })
