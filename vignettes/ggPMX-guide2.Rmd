...
output:
  ...
  word_document:
    toc: yes
    toc_depth: '3'
vignette: "%\\VignetteIndexEntry{ggPMX: User guide} \n%\\VignetteEngine{knitr::rmarkdown}
...
**Something else to highlight? Amine to explain a bit more.**
...
ctr %>% plot_names
```
Note here the use of piping. An alternative way to display the names of the existing plots is by printing the content of the controller as done in the previous section. or to use :
...
`ggPMX` comes with a list of plot types. This is not to be confused with plot names. Each plot type can be used to create one or many plots.
```{r plot_types}
 ctr %>% plots
```
...
p1 = ctr %>% 
  set_plot("RES", pname = "pred_vs_time",x="TIME",y="PRED",strat.facet = "STUD",
...
To update some plot features and to access additional settings, we use `pmx_update()`. Note that `pmx_update` changes permanently the actual content of the controller.
...
```
or even simpler by using `pmx_plot_individual`:

```{r indiv_plots_update_defualt, fig.height=3, fig.width=5, fig.align='center'}
ctr %>% pmx_plot_individual(npage=1,labels=list(title="My individuals"))
```
...
```{r indiv_plots_grid_default, fig.height=3.5, fig.width=6, fig.align='center'}
ctr %>% pmx_plot_individual(npage=1,
                  facets=list(nrow=2,ncol=4),
                  point=list(shape=4,color="red"),
                  labels=list(title="My individual plots grid"))
...
p1 <- ctr %>% get_plot("indiv1",npage=1)

p2 <- ctr %>% pmx_plot_individual(1,
  labels=list(x ="Time (days)",
...
# Default dv_pred plot
p1 <- ctr %>% pmx_plot_dv_pred

# Change x- and y-labels (doesn't work with Rmarkdown, strange...)
p2 <- ctr %>% pmx_plot_dv_pred(, 
                         labels=list(x="Population predictions", y="Observations"), 
                         add_hline=FALSE,point=list(color="green4")) 



# modify the colors and shapes of points
# Change x- and y-labels (doesn't work with Rmarkdown, strange...)
...
p3 <- ctr %>% pmx_plot_dv_pred(
  ...
  identity_line=list(color="red"),
  has.smooth=TRUE,
  point=list(color="green4",shape=4))
...
p2 <- ctr %>% 
  pmx_plot_iwres_ipred( has.band = TRUE, 
...
ctr <- 
  pmx_mlx(
    ...
    conts=c("WT0","AGE0"),
    strats="STUD",
    settings = list(is.draft=FALSE)) ## here the draft option
ctr %>% pmx_plot_dv_pred()
```


(I find it difficult to know what are the existing fields of pmx_update. They seem to be different according to which plot type the function is applied. Could we list the mandatory and optional arguments to this function?)
...
```{r}
ctr = theophylline()
shrinkage(ctr %>% get_data("estimates"),
          ctr %>% get_data("eta"))
```
...
```{r}
ctr %>% 
  pmx_update("ebe_box",has.shrink=TRUE) %>%   
  get_plot("ebe_box")
```
...
ctr %>% pmx_update("ebe_hist",has.shrink=TRUE,strat.facet=~SEX) %>% get_plot("ebe_hist")
```
...
Standing. Are there others?

\clearpage 

# Appendix B: All Theophylline plots

We provide here the list of all plots of the theophylline example.
...
## EBE plots
<!-- ```{r th_plots_others, out.width='.49\\linewidth', fig.width=4, fig.height=3,fig.show='hold',fig.align='left',message=FALSE} -->
<!-- yy <- ctr %>% plots -->
