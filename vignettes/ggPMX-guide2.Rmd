...
#   pmx_update(
#     "indiv",
#     labels=list(xlab="Time (days)", ylab="Free serum concentration (nmol)")) %>%
#   get_plot("indiv",npage=1)

# An alternative way:
p2 <- ctr %>% get_plot("indiv1",npage=1) +
  xlab("Time (days)") +
  ylab("Free serum concentration (nmol)")
p1
p2
...
ctr %>% set_plot("RES", pname = "res2", x = "PRED", y = "DV")
p1 <- ctr %>% get_plot("res2")

# Change x- and y-labels (doesn't work with Rmarkdown, strange...)
p2 <- ctr %>% pmx_update("res2", 
                         labels=list(x="Population predictions", y="Observations"), 
                         add_hline=FALSE,point=list(color="green4")) %>%
  get_plot("res2")

# How do I modify the colors and shapes of points? What are the possible values of 'point' argument? I can't find it in full documentation.
...
ctr %>% set_plot("RES", pname = "res1", x = "IPRED", y = "IWRES")
