...
theophylline <- file.path(system.file(package = "ggPMX"), 
                          "testdata", "theophylline")
WORK_DIR <-  file.path(theophylline, "Monolix")
input_file <- file.path(theophylline,"data_pk.csv")
pmxOptions(
  work_dir=WORK_DIR,
...
The `shrinkage` function allows $\epsilon$-shrinkage to be calculated using the parameter and individual estimation data. $\epsilon$-shrinkage is calculated as \[
1-SD(\epsilon)/\omega 
\]
where $\epsilon$ are the between individual variation terms and $\omega$ is the population model estimate of the standard deviation in $\epsilon$.
