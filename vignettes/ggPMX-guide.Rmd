...
* The *output modeling datasets* are those output from the fitting tool (Monolix, NONMEM or nlmixr). For version 1.0 of ggPMX, these datasets have to follow the structure of outputs of Monolix 2016 or 2018 (See Appendix for more details on Monolix requirements).
...
vpc_file <- write.csv(simdata2, file = "my_VPC.csv", quote=FALSE, row.names = FALSE)
```
