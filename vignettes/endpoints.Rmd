...
A model aims at predicting one (e.g., PK), two (e.g., PK and PD) endpoints. The measured value of the endpoint is denoted by DV.  IPRED and PRED are prediction for the endpoint.
Each report is about one single endpoint. The endpoint (if specified) should in the controller call to keep only the corresponding values.
...
- **code** (integer or charcater :how the endpoint is coded)
- **label** (can be used in some plots title or the report file name)
- **unit**  (used in the axis label for some plots)
- **prefix** output filename prefix (use for filename (Section 3.4)), or list of output filenames 
