...
```{r}
ctr %>% pmx_plot_ebe_box(is.shrink=FALSE)
ctr %>% pmx_plot_ebe_hist(is.shrink=FALSE)
...
```{r}
ctr %>% pmx_plot_ebe_hist(filter=EFFECT %in% c("ka","Cl"))
```
...
### Draft label (default = T)
```{r}
ctr %>% pmx_plot_ebe_hist(is.draft=FALSE)
```

### Histogram fill (default=white)

```{r}
ctr %>% pmx_plot_ebe_hist(histogram=list(fill="gray"))
```

### Histogram color (default = black)
```{r}
ctr %>% pmx_plot_ebe_hist(histogram=list(fill='green',color='green'))
...
```{r , warning=FALSE}
ctr %>% pmx_plot_ebe_hist(ranges=list(x=c(-0.2,0.2)))
ctr %>% pmx_plot_ebe_box(ranges=list(y=c(-0.1,0.2)))
```

### X and y axis labels (default = absent)

```{r}
ctr %>% pmx_plot_ebe_box(labels=list(x="X_LAB",y="YLAB"))
```
...
```{r}
ctr %>% pmx_plot_ebe_hist(strat.facet=~STUD)
ctr %>% pmx_plot_ebe_hist(strat.color="STUD")
ctr %>% pmx_plot_ebe_box(strat.color="STUD",shrink=list(annotation="sh"))
ctr %>% pmx_plot_ebe_box(strat.facet=~STUD, shrink=list(annotation="sh"))
