...
ctr <- theophylline(settings=pmx_settings(use_finegrid = FALSE))
...
## Define catgerical labels :

In case of faceting by stratification user can redfine categrical labels to have more human readables strips. Lables are defined within `cats.labels` argument and user can use them by setting `use_lables` to TRUE.
