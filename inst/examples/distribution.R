...
## add jitter
ctr %>% pmx_plot_ebe_box(has.jitter=TRUE,jitter=list(alpha=0.4,color="red"))

## remove shrinkage 
ctr %>% pmx_plot_ebe_hist(has.shrink=FALSE)

## updat histogram graphical parameters
ctr %>% pmx_plot_ebe_hist(histogram=list(color=NA,position="fill",binwidth=1/100))
